...
//Syntax 2: //tageName[text()="text-value"]
...
test('Xpath Locator', async({ page }) => {
    ...
    // Locate the 'Para Bank' logo image  using Syntax 1
    await page.locator('//img[@title="ParaBank"]');

    // Locate the 'Caption' text using Syntax 2
    await page.locator('//p[text()="Experince the difference"]');

    // Locate the 'Para Bank' logo image  using Syntax 3
