...
export class LoginPageSteps {
    ...
    async launchtheApplication(): Promise<void> {
        await this.web.launchTheApplication(config.app.url,config.app.title);
    }
    ...
    async enterBusinessEmailAndPassword(email: string, password?: string) {
        await this.web.typeTextWithinTextBox(loginPage.businessEmailTextBox, email);
        if (password) {
            await this.web.typeTextWithinTextBox(loginPage.passwordTextBox, password);
        }
    }
